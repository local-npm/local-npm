...
module.exports = function (options, callback) {
  ...
  app.get('/tarballs/:name/:version.tgz', function (req, res) {
    ...
    .then(function(doc) {
      ...
      db.get(id, { asBuffer: true, valueEncoding: 'binary' }, function (err, buffer) {
        ...
        if(dist.info) {
            logger.time('request.get(' + dist.info + ')');
            var infoBuffs = [];
            request.get(dist.info).on('error', function (err) {
                logger.info(err);
                res.status(500).send('you are offline and this package isn\'t cached');
            }).pipe(through(function (chunk, _, next) {
                infoBuffs.push(chunk);
                next();
            }, function (next) {
                next();
                var info = JSON.parse(Buffer.concat(infoBuffs));

                var buffs = [];
                request.get(info.dist.tarball).on('error', function (err) {
                    logger.info(err);
                    res.status(500).send('you are offline and this package isn\'t cached');
                }).pipe(through(function (chunk, _, next) {
                    buffs.push(chunk);
                    ...
                    logger.timeEnd('request.get(' + dist.info + ')');
    ...
    .then(function() {
      return skimLocal.get(pkgName).then(function(doc) {
  ...
  app.get('/tarballs/:user/:package/:version.tgz', function (req, res) {
      ...
      logger.time('2: skimLocal.get(' + fullName + ')');
      ...
      .then(function(doc) {
        logger.timeEnd('2: skimLocal.get(' + fullName + ')');
        var dist = doc.versions[pkgVersion].dist;
        logger.time('db.get(' + id + ')');
        db.get(id, { asBuffer: true, valueEncoding: 'binary' }, function (err, buffer) {
          logger.timeEnd('db.get(' + id + ')');
          ...
          logger.miss(fullName, pkgVersion);
          ...
          if(dist.info) {
              ...
              request.get(dist.info).on('error', function (err) {
                  ...
                  request.get(info.dist.tarball).on('error', function (err) {
                      ...
                      sendBinary(res, buffer);
                      db.put(id, buffer, function (err) {
                          logger.cached(fullName, pkgVersion);
                          if (err) {
                              logger.info(err);
                          }
                      });
      ...
      .then(function() {
        return skimLocal.get(fullName).then(function(doc) {
          doc.versions[pkgVersion].downloads ? doc.versions[pkgVersion].downloads += 1 : doc.versions[pkgVersion].downloads = 1;
          return skimLocal.put(doc);
        });
      }).catch(function (err) {
